const jobs = [
  ...
  { score: 96, name: "大语言模型研究员", org: "厦门云知芯 / 云知声", city: "厦门", track: "LLM Engineering", skills: "RAG · Agent · PyTorch · Megatron · DeepSpeed", url: "https://www.xmsoft.com/pzt/fg/recruitEntDetail.html?parkCd=02&recruitId=8a8180828df42c8e018df449dd150158" },
  { score: 86, name: "强化学习算法工程师", org: "厦门渊亭", city: "厦门 / 北京", track: "Model Algorithm", skills: "RL · Python · LLM · 模型评测", url: "https://www.utenet.com/recruit" },
  { score: 85, name: "AI驱动设计算法工程师", org: "Amphenol", city: "厦门", track: "Model Algorithm", skills: "Python · 生成式3D · CAD/CAE · 优化", url: "https://www.amphenol-ast.com/v3/cn/careers_view.aspx?id=528" },
  { score: 90, name: "AI 开发工程师", org: "开锐咨询", city: "厦门", track: "Agent Engineering", skills: "LLM · RAG · Agent · 后端集成 · 评测", url: "https://www.krc.com.cn/careers/ai-developer" },
];
...
const officialPipelines = [
  ...
  ["云知声", "厦门", "LLM · RAG · Agent · 分布式训练", "https://www.xmsoft.com/pzt/fg/recruitEntDetail.html?parkCd=02&recruitId=8a8180828df42c8e018df449dd150158"],
  ["渊亭科技", "厦门 / 北京", "强化学习 · 大模型算法", "https://www.utenet.com/recruit"],
  ["Amphenol", "厦门", "生成式 3D · CAD/CAE · AI 优化", "https://www.amphenol-ast.com/v3/cn/careers_view.aspx?id=528"],
  ["开锐咨询", "厦门", "LLM · RAG · Agent · 系统集成", "https://www.krc.com.cn/careers/ai-developer"],
];
...
export default function Home() {
  return <main>
    ...
    <section id="top" className="wrap intro"><p className="kicker">PERSONAL CAREER INTELLIGENCE</p><h1>用真实机会，<em>组织学习与行动。</em></h1><p>开发、AI Infra、模型算法、智能体方向的双雷达。岗位池由可直达职位与官方招聘方向组成，避免把搜索结果伪装为职位。</p><div className="stats"><div><b>63+</b><span>可行动岗位池</span></div><div><b>16</b><span>开源机会</span></div><div><b>37</b><span>官方来源</span></div><div><b>5</b><span>优先城市</span></div></div></section>
    <section id="jobs" className="wrap section"><div className="section-title"><div><p className="kicker">01 / JOB RADAR</p><h2>优先投递</h2></div><p>城市排序：深圳 → 广州 → 上海 → 北京 → 厦门；新加坡仅作补充观察。</p></div><div className="job-list">{jobs.map((job) => <article className="job" key={job.url + job.name}><b className="score">{job.score}</b><div><h3>{job.name}</h3><p>{job.org} · {job.city} · {job.track}</p><span>{job.skills}</span></div><a href={job.url} target="_blank" rel="noreferrer" aria-label={`打开 ${job.name} 官方岗位详情`}>官方详情 ↗</a></article>)}</div><p className="hint">以上为 21 个已核验职位；厦门优先展示云知声、渊亭、美图、艾飞、安费诺与开锐方向，其余已核验记录保留在数据报告中。</p><h3 className="pool-title">官方岗位方向池 · 50 条</h3><p className="hint">每家保留两个与你匹配的投递方向，链接均为公司官方招聘入口；进入后按城市与关键词筛选再投递。</p><div className="pipeline-grid">{officialPipelines.map(([company, city, focus, url]) => <a href={url} target="_blank" rel="noreferrer" key={company}><b>{company}</b><span>{city}</span><p>{focus}</p><small>2 个候选方向 · 官方入口 ↗</small></a>)}</div></section>
