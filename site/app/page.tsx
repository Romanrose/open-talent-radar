...
export default function Home() {
  return <main>
    ...
    <section id="top" className="wrap intro"><p className="kicker">PERSONAL CAREER INTELLIGENCE</p><h1>用真实机会，<em>组织学习与行动。</em></h1><p>开发、AI Infra、模型算法、智能体方向的双雷达。岗位池由可直达职位与官方招聘方向组成，避免把搜索结果伪装为职位。</p><div className="stats"><div><b>59+</b><span>可行动岗位池</span></div><div><b>16</b><span>开源机会</span></div><div><b>35</b><span>官方来源</span></div><div><b>5</b><span>优先城市</span></div></div></section>
    <section id="jobs" className="wrap section"><div className="section-title"><div><p className="kicker">01 / JOB RADAR</p><h2>优先投递</h2></div><p>城市排序：深圳 → 广州 → 上海 → 北京 → 厦门；新加坡仅作补充观察。</p></div><div className="job-list">{jobs.map((job) => <article className="job" key={job.url + job.name}><b className="score">{job.score}</b><div><h3>{job.name}</h3><p>{job.org} · {job.city} · {job.track}</p><span>{job.skills}</span></div><a href={job.url} target="_blank" rel="noreferrer" aria-label={`打开 ${job.name} 官方岗位详情`}>官方详情 ↗</a></article>)}</div><p className="hint">以上为 19 个已核验职位；厦门优先展示云知声、渊亭、美图与艾飞方向，其余已核验记录保留在数据报告中。</p><h3 className="pool-title">官方岗位方向池 · 46 条</h3><p className="hint">每家保留两个与你匹配的投递方向，链接均为公司官方招聘入口；进入后按城市与关键词筛选再投递。</p><div className="pipeline-grid">{officialPipelines.map(([company, city, focus, url]) => <a href={url} target="_blank" rel="noreferrer" key={company}><b>{company}</b><span>{city}</span><p>{focus}</p><small>2 个候选方向 · 官方入口 ↗</small></a>)}</div></section>
